...
frappe.ui.form.on('Depense Interne', {
    ...
    refresh(frm) {
        ...
        if (frm.doc.article) {
            ...
            frappe.db.get_value('Budget Article', frm.doc.article, 'has_partition', (r) => {
    ...
    chapitre(frm) {
        if (frm.doc.chapitre) {
            frappe.db.get_value('Budget Chapitre', frm.doc.chapitre, 'intitule', r => {
    ...
    article(frm) {
        ...
        frappe.db.get_value('Budget Article', frm.doc.article,
        	['code_article', 'intitule_article', 'budget_chapitre', 'type'], r => {
            	...
            	if (r.type !== 'A posteriori') {
                    ...
                    frm.set_value('chapitre', r.budget_chapitre);
                    frm.set_value('code_partition', r.code_partition || '');
                    frm.set_value('intitule_partition', r.intitule_partition || '');
    ...
    toggle_partition_fields(frm) {
        if (frm.has_partition) {
            frm.set_df_property('code_partition', 'hidden', 0);
            frm.set_df_property('intitule_partition', 'hidden', 0);
        } else {
            frm.set_df_property('code_partition', 'hidden', 1);
